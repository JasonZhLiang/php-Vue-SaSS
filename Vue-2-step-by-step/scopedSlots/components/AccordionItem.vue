...
<script>
    export default {
        ...
        methods: {
            toggle() {
                this.$emit('toggled', this.$vnode.key);//this will emit an event and pass the current key of item.
                console.log('the accordion item with an index of ' + this.$vnode.key + ' was toggled.');
            }
        }
    };
</script>
